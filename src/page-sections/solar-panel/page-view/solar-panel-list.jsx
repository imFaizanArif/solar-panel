...
const SolarPanelListPageView = () => {
  ...
  const handleSearch = (event) => {
    ...
    setSearchTerm(value);
    if (value === '') {
      setFilteredData(solarPanelData);
    } else {
      const filteredData = solarPanelData.filter((item) =>
          ...
          <>
            ...
            >
              <MenuItem
                onClick={() => {
                  handleCloseOpenMenu();
                  navigate(`/dashboard/update-solar-panel/${record.id}`);
                }}
              >
  ...
  return <Card>
    ...
    <FlexBetween flexWrap="wrap" gap={2} p={2} pt={2.5}>
      <Stack direction="row" alignItems="center">
        ...
        <H5 fontSize={16}>Solar Panel List</H5>
      </Stack>

      <Button variant="contained" startIcon={<Add />} onClick={() => navigate("/dashboard/create-solar-panel")}>
        Add Solar Panel
      </Button>
    </FlexBetween>
    ...
    <Wrapper gap={2} px={2} pb={3}>
      ...
      <TextField
        fullWidth
        label="Search solar panel by name..."
        value={searchTerm}
    ...
    <div>
      ...
      >
        <Box sx={style}>
          <H6 fontSize={18}>Delete Solar Panel</H6>
...
export default SolarPanelListPageView;