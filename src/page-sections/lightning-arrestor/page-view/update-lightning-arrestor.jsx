...
const UpdateLightningArrestorPageView = () => {
  const { id } = useParams();
  ...
  const handleCancel = () => navigate("/dashboard/lightning-arrestor-list");
  const getLightningArrestorList = async () => {
    try {
      ...
      const LightningArrestor = LightningArrestorData.find((panel) => {
        return panel.id === LightningArrestorId;
      });
      if (LightningArrestor) {
        setInitialValues({
          name: LightningArrestor.name,
          price: LightningArrestor.price,
        });
      }
    } catch (err) {
      console.log(err.response.data);
      toast.error("Failed to fetch Lightning Arrestor data");
    }
  };
      ...
      <Formik initialValues={initialValues} validationSchema={validationSchema}
        ...
        onSubmit={async (values, { setSubmitting }) => {
          ...
          try {
            ...
            if (res.status == 200) {
              toast.success("Lightning Arrestor Updated Successfully");
              setloading(false);
              setTimeout(() => {
                navigate("/dashboard/lightning-arrestor-list");
              }, 1000);
            }
          } catch (err) {
            setloading(false);
...
export default UpdateLightningArrestorPageView;