...
const InvoiceListPageView = () => {
    ...
    const columns = [
      {
        ...
        key: "date",
        render: (text, record) => {
          return <span>{formatNumber(record.discount)}</span>;
        },
      },
      {
        ...
        key: "name",
        render: (text, record) => {
          return <span>{formatNumber(record.shipping_charges)}</span>;
        },
      },
      {
        title: "Solar Panel",
        children: [
          ...
          },
          {
            ...
            key: "name",
            render: (text, record) => {
              return <span>{formatNumber(record.solar_panel_quantity)}</span>;
            },
          },
          {
            title: "Price",
            ...
            width: 150,
            render: (text, record) => {
              return <span>{formatNumber(record.solar_panel_price)}</span>;
            },
          },
        ],
      },
      {
        ...
        children: [
          ...
          },
          {
            ...
            key: "name",
            render: (text, record) => {
              return <span>{formatNumber(record.inverter_quantity)}</span>;
            },
          },
          {
            title: "Price",
            ...
            width: 150,
            render: (text, record) => {
              return <span>{formatNumber(record.inverter_price)}</span>;
            },
          },
        ],
      },
      {
        ...
        children: [
          ...
          },
          {
            ...
            key: "name",
            render: (text, record) => {
              return <span>{formatNumber(record.cabling_quantity)}</span>;
            },
          },
          {
            title: "Price",
            ...
            width: 150,
            render: (text, record) => {
              return <span>{formatNumber(record.cabling_price)}</span>;
            },
          },
        ],
      },
      {
        ...
        children: [
          ...
          },
          {
            ...
            key: "name",
            render: (text, record) => {
              return <span>{formatNumber(record.structure_quantity)}</span>;
            },
          },
          {
            title: "Price",
            ...
            width: 150,
            render: (text, record) => {
              return <span>{formatNumber(record.structure_price)}</span>;
            },
          },
        ],
      },
      {
        ...
        children: [
          ...
          },
          {
            ...
            render: (text, record) => {
              return <span>{formatNumber(record.net_metering_quantity)}</span>;
            },
          },
          {
            title: "Price",
            ...
            render: (text, record) => {
              return <span>{formatNumber(record.net_metering_price)}</span>;
            },
          },
        ],
      },
      {
        ...
        children: [
          ...
          },
          {
            ...
            render: (text, record) => {
              return <span>{formatNumber(record.battery_quantity)}</span>;
            },
          },
          {
            title: "Price",
            ...
            key: "battery_price",
            render: (text, record) => {
              return <span>{formatNumber(record.battery_price)}</span>;
            },
          },
        ],
      },
      {
        ...
        children: [
          ...
          },
          {
            ...
            render: (text, record) => {
              return <span>{formatNumber(record.installation_quantity)}</span>;
            },
          },
          {
            title: "Price",
            ...
            render: (text, record) => {
              return <span>{formatNumber(record.installation_price)}</span>;
            },
          },
        ],
      },
      ...
      <Table
        ...
        expandable={{
          ...
          rowExpandable: (record) =>
            record.partial_payments && record.partial_payments.length > 0,
        }}
        scroll={{ x: 4200 }}
      />
    );
  ...
  const columns = [
          ...
          <span style={{ fontWeight: 600 }}>
            {formatNumber(
              parseInt(record.total) +
                parseInt(record.shipping_charges) -
                parseInt(record.discount)
            )}
          </span>
        );
      },
      ...
      render: (text, record) => {
        ...
        return (
          <span style={{ color: "green", fontWeight: 600 }}>
            {sum !== undefined ? formatNumber(parseInt(sum)) : "Loading..."}
          </span>
        );
      },
      sorter: {
    ...
    },
    {
      ...
      render: (invoiceId) => {
        ...
        return (
          <Link to={`/dashboard/expenditure-list`} style={{ color: "black" }}>
            {totalExpenditure}
          </Link>
        );
      },
      ...
      render: (text, record) => {
        ...
        return (
          <>
            <Button
              aria-controls={
                openMenuEl === record?.id ? "simple-menu" : undefined
              }
              aria-haspopup="true"
              color="inherit"
              variant="text"
            ...
            >
              ...
              >
                <Payment fontSize="16" color="primary" />
                &nbsp; Add Expenditure
              </MenuItem>
              <MenuItem
                onClick={() => {
              ...
              >
                <Download fontSize="16" color="success" />
                &nbsp; Download
              </MenuItem>
              <MenuItem
                onClick={() => {
              ...
              >
                <DeleteOutline fontSize="16" color="error" />
                &nbsp; Delete
              </MenuItem>
            </Menu>
          </>
  ...
  const getInvoiceList = () => {
    axios
      .get(baseApiUrl + "/invoice")
      .then((res) => {
        // console.log(res.data)
        setInvoiceData(res?.data.reverse());
        setFilteredData(res?.data);
        setInvoiceDeatiledData(
          res?.data?.filter((invoice) => invoice.id === rowId)
        );
        const partial_payments = res?.data?.filter(
      ...
      const payableAmounts = invoiceData?.map((invoice) => {
        ...
        return {
          ...
          payable:
            ...
            sumOfPayments +
            parseFloat(invoice.shipping_charges),
        };
      });
  ...
  const getInvoiceFilteredList = (startDate, endDate, querry) => {
    axios
      .get(
        baseApiUrl +
          `/api/Invoice?date_range=${startDate}+to+${endDate}&q=${querry}`
      )
      .then((res) => {
  ...
  const getExpendituresList = async () => {
    try {
      ...
      setSpecificExpendituresData(
        ExpendituresData.filter((expenditure) => expenditure.inv_id === query)
      );
    } catch (err) {
      console.log(err.response.data);
    }
  ...
  const deleteInvoice = (id) => {
    axios
      .delete(baseApiUrl + `/api/Invoice/${id}/`)
      .then((res) => {
        if (res.status == 204) {
          getInvoiceList();
  ...
  const downloadInvoice = async (recordId) => {
    try {
      const response = await axios.get(
        `${baseApiUrl}/invoice/${recordId}/download`,
        {
  ...
  const handleDateRangeChange = (value) => {
    if (value == null || value == "") {
      ...
      const formattedDatesArray = value.map(
  ...
  const totalSumExpenditure = specificExpendituresData.reduce(
