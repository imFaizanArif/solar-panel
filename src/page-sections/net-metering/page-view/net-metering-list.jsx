...
const NetMeteringListPageView = () => {
  ...
  const handleSearch = (event) => {
    ...
    setSearchTerm(value);
    if (value === '') {
      setFilteredData(NetMeteringData);
    } else {
      const filteredData = NetMeteringData.filter((item) =>
          ...
          <>
            ...
            >
              <MenuItem
                onClick={() => {
                  handleCloseOpenMenu();
                  navigate(`/dashboard/update-net-metering/${record.id}`);
                }}
              >
  ...
  const deleteNetMetering = (id) => {
    axios.delete(baseApiUrl + `${id}/`)
      .then((res) => {
        if (res.status == 204) {
          getNetMeteringList();
          toast.success("Net Metering Deleted Successfully");
        }
      })
      .catch((err) => {
  ...
  return <Card>
    ...
    <FlexBetween flexWrap="wrap" gap={2} p={2} pt={2.5}>
      <Stack direction="row" alignItems="center">
        ...
        <H5 fontSize={16}>Net Metering List</H5>
      </Stack>

      <Button variant="contained" startIcon={<Add />} onClick={() => navigate("/dashboard/create-net-metering")}>
        Add Net Metering
      </Button>
    </FlexBetween>
    ...
    <Wrapper gap={2} px={2} pb={3}>
      ...
      <TextField
        fullWidth
        label="Search Net Metering by name..."
        value={searchTerm}
    ...
    <div>
      ...
      >
        <Box sx={style}>
          <H6 fontSize={18}>Delete Net Metering</H6>
...
export default NetMeteringListPageView;