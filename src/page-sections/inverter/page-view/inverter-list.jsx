...
const InverterListPageView = () => {
  ...
  const handleSearch = (event) => {
    ...
    setSearchTerm(value);
    if (value === '') {
      setFilteredData(InverterData);
    } else {
      const filteredData = InverterData.filter((item) =>
          ...
          <>
            ...
            >
              <MenuItem
                onClick={() => {
                  handleCloseOpenMenu();
                  navigate(`/dashboard/update-inverter/${record.id}`);
                }}
              >
  ...
  return <Card>
    ...
    <FlexBetween flexWrap="wrap" gap={2} p={2} pt={2.5}>
      <Stack direction="row" alignItems="center">
        ...
        <H5 fontSize={16}>Inverter List</H5>
      </Stack>

      <Button variant="contained" startIcon={<Add />} onClick={() => navigate("/dashboard/create-inverter")}>
        Add Inverter
      </Button>
    </FlexBetween>
    ...
    <div>
      ...
      >
        <Box sx={style}>
          <H6 fontSize={18}>Delete Inverter</H6>
...
export default InverterListPageView;