...
const BatteriesListPageView = () => {
  ...
  const handleSearch = (event) => {
    ...
    setSearchTerm(value);
    if (value === '') {
      setFilteredData(BatteriesData);
    } else {
      const filteredData = BatteriesData.filter((item) =>
          ...
          <>
            ...
            >
              <MenuItem
                onClick={() => {
                  handleCloseOpenMenu();
                  navigate(`/dashboard/update-batteries/${record.id}`);
                }}
              >
  ...
  const deleteBatteries = (id) => {
    axios.delete(baseApiUrl + `${id}/`)
      .then((res) => {
        if (res.status == 204) {
          getBatteriesList();
          toast.success("Batteries Deleted Successfully");
        }
      })
      .catch((err) => {
  ...
  return <Card>
    ...
    <FlexBetween flexWrap="wrap" gap={2} p={2} pt={2.5}>
      <Stack direction="row" alignItems="center">
        ...
        <H5 fontSize={16}>Batteries List</H5>
      </Stack>

      <Button variant="contained" startIcon={<Add />} onClick={() => navigate("/dashboard/create-batteries")}>
        Add Batteries
      </Button>
    </FlexBetween>
    ...
    <Wrapper gap={2} px={2} pb={3}>
      ...
      <TextField
        fullWidth
        label="Search batteries by name..."
        value={searchTerm}
    ...
    <div>
      ...
      >
        <Box sx={style}>
          <H6 fontSize={18}>Delete Batteries</H6>
...
export default BatteriesListPageView;